...
type Analytics = {
  ...
  accessByAction: Array<{ _id: string; count: number }>;
  topUsers: Array<{ _id: string; count: number }>;
  datasetActivity: Array<{ _id: string; count: number }>;
};
...
export default function AccessLogsPage() {
  ...
  async function fetchAccessLogs() {
    try {
      ...
      const filters: any = {};
      if (dataTypeFilter !== "all") filters.dataType = dataTypeFilter;
      if (actionFilter !== "all") filters.action = actionFilter;
      if (statusFilter !== "all") filters.status = statusFilter;
      if (datasetFilter !== "all") filters.datasetName = datasetFilter;

      const [logsRes, analyticsRes] = await Promise.all([
        apiMethods.getAccessLogs(0, 100, filters),
  ...
  const distinctDatasets = useMemo(() => {
    return Array.from(
      new Set(logs.map((l) => l.datasetName?.trim()).filter(Boolean))
    ) as string[];
  }, [logs]);
      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600">
              Monitor and audit user interactions with healthcare datasets and records
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <StatCard
            ...
            value={analytics?.failedAccesses || 0}
            tone="orange"
          />
          <StatCard
            label="Datasets Accessed"
            value={analytics?.datasetActivity?.length || 0}
            tone="blue"
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg border shadow-sm p-6">
            <h2 className="font-semibold text-slate-900 mb-4">Access by Action</h2>
            <div className="space-y-3">
              {analytics?.accessByAction?.length ? (
                analytics.accessByAction.map((item, index) => (
                  <div key={index} className="flex items-center justify-between border-b pb-3">
                    <span className="text-slate-700 uppercase">{item._id || "Unknown"}</span>
                    <span className="font-semibold text-slate-900">{item.count}</span>
                  </div>
                ))
              ) : (
        ...
        <div className="bg-white rounded-lg border shadow-sm p-6">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            <input
              type="text"
              placeholder="Search user, action, dataset, record..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500 md:col-span-2"
            />
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="text-left px-6 py-4 font-semibold text-slate-700">Record / Resource</th>
                ...
                <tbody>
                  {filteredLogs.map((log, index) => (
                    <tr key={log._id || index} className="border-t hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        {log.recordId || log.resourceId || "N/A"}
                      </td>
                      <td className="px-6 py-4 text-slate-600">
                        {log.ipAddress || "N/A"}
                      </td>
